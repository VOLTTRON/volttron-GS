...
mTN = steam_loop
mTN.description = 'hot steam distribution loop on campus'
mTN.name = 'steam_loop'

# set up meter point
heat_meter = MeterPoint()
MP = heat_meter
MP.description = 'meters mass flowrate through hot steam loop'
MP.measurementType = MeasurementType.Heat
MP.measurement = MeasurementUnit.kWh
steam_meter = MP

# provide a cell array of all Meterpoints to myTransactiveNode
mTN.meterpoints = [steam_meter]
...
## Day Ahead Market
dayAhead = Market(measurementType = [MeasurementType.Heat])
MKT = dayAhead
MKT.name = 'steamloop_Market'
...
TUR111Model.receive_transactive_signal(steam_loop)
TUR115Model.receive_transactive_signal(steam_loop)
TVW131Model.receive_transactive_signal(steam_loop)
SPU122Model.receive_transactive_signal(steam_loop)
SPU124Model.receive_transactive_signal(steam_loop)
SPU125Model.receive_transactive_signal(steam_loop)
...
TUR111Model.prep_transactive_signal(dayAhead, steam_loop)
TUR115Model.prep_transactive_signal(dayAhead, steam_loop)
TVW131Model.prep_transactive_signal(dayAhead, steam_loop)
SPU122Model.prep_transactive_signal(dayAhead, steam_loop)
SPU124Model.prep_transactive_signal(dayAhead, steam_loop)
SPU125Model.prep_transactive_signal(dayAhead, steam_loop)

# Finally, the prepared TransactiveRecords are sent to their corresponding
# TransactiveNeighbor.
TUR111Model.send_transactive_signal(steam_loop)
TUR115Model.send_transactive_signal(steam_loop)
TVW131Model.send_transactive_signal(steam_loop)
SPU122Model.send_transactive_signal(steam_loop)
SPU124Model.send_transactive_signal(steam_loop)
SPU125Model.send_transactive_signal(steam_loop)

# invoke the market object to sum all powers as will be needed by the 
# net supply/demand curve
dayAhead.assign_system_vertices(steam_loop)

# view the system supply/demand curve
dayAhead.view_net_curve(0, MeasurementType.Heat)